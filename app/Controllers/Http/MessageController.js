...
class MessageController {
   ...
   * @param {Response} ctx.response
   */
  async store ({ request, response, auth: { user: { id, username } } }) {
    const { text } = request.only(['text'])

    const channel = await Channel.find(1)

    const message = await channel.messages().create({ text, user_id: id, user_username: username })
